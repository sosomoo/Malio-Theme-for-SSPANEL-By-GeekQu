<div class="card-inner">
    <p class="card-heading">输入充值金额后，点击下方的图标进行充值</p>
    <div class="form-group form-group-label">
        <label class="floating-label" for="amount">金额</label>
        <input class="form-control" id="amount" type="text">
    </div>
</div>
<div id="ymqqrarea">
    <button class="btn btn-flat waves-attach" id="ymq-alipay-submit" name="type" onclick="pay('alipay')"><img
                src="data:image/svg+xml;base64,"
                width="64" height="64"></button>
    <button class="btn btn-flat waves-attach" id="ymq-wechat-submit" name="type" onclick="pay('wechat')"><img
                src="data:image/svg+xml;base64,"
                width="64" height="64"></button>
</div>
<script>
    var pid = 0;
    var count_down = 0;
    function pay(type) {
        var price = parseFloat($$getValue('amount'));
        //console.log("将要使用 " + type + " 充值" + price + "元");
        if (isNaN(price)) {
            $("#readytopay").modal('hide');
            $("#result").modal();
            $$.getElementById('msg').innerHTML = '非法的金额！'
            return;
        }
        $('#readytopay').modal();
        $("#readytopay").on('shown.bs.modal', function () {
            $.ajax({
                url: "/user/payment/purchase",
                data: {
                    price,
                    type,
                },
                dataType: 'json',
                type: "POST",
                success: (data) => {
                    if (data.code == 200) {
                        //console.log(data);
                        $("#readytopay").modal('hide');
                        {
                            pid = data.pid;
                            count_down= data.expire_in;
                            if (type=='alipay'){
                                $$.getElementById('ymqqrarea').innerHTML = '<div class="text-center"><p>使用支付宝扫描二维码支付</p><p id="countdown">将于'+count_down+'秒后失效</p><div align="center" id="qrcode" style="padding-top:10px;"></div><p>充值完毕后会自动跳转</p></div>';
                            }else {
                                $$.getElementById('ymqqrarea').innerHTML = '<div class="text-center"><p>使用微信扫描二维码支付</p><p id="countdown">将于'+count_down+'秒后失效</p><div align="center" id="qrcode" style="padding-top:10px;"></div><p>充值完毕后会自动跳转</p></div>';

                            }
                            var qrcode = new QRCode("qrcode", {
                                correctLevel: 3,  //解决超过200字符的二维码生成问题
                                render: "canvas",
                                width: 200,
                                height: 200,
                                text: data.url      //使用encodeURI()函数报会调低至错误
                            });
                            tid = setTimeout(f, 1000); //循环调用触发setTimeout
                            tdd = setTimeout(down,1000);
                        }
                    } else {
                        $("#result").modal();
                        $$.getElementById('msg').innerHTML = data.msg;
                        //console.log(data);
                    }
                }
            });
        });
    }
    function down(){
        count_down = count_down-1;
        $$.getElementById('countdown').textContent="将于"+count_down+"秒后失效";
        if (count_down === 0){
            $$.getElementById('ymqqrarea').innerHTML='<div class="text-center"><p>二维码失效,请重新发起</p></div>';
        }
        tdd = setTimeout(down,1000);
    }
    function f() {
        $.ajax({
            type: "POST",
            url: "/payment/status",
            dataType: "json",
            data: {
                pid
            },
            success: (data) => {
                if (data.result) {
                    //console.log(data);
                    $("#result").modal();
                    $$.getElementById('msg').innerHTML = '充值成功！';
                    window.setTimeout("location.href=window.location.href", {$config['jump_delay']});
                }
            },
            error: (jqXHR) => {
                //console.log(jqXHR);
            }
        });
        tid = setTimeout(f, 1000); //循环调用触发setTimeout

    }
</script>
